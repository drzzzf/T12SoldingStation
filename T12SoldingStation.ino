...
#define ONSTAND_PIN   A6        // handle on stand sensor
...
uint16_t  SetTemp, ShowTemp, gap, Step, HandleDistance;
...
void SLEEPCheck() {  
  if (handleMoved && !isOnStand) {                    // if handle was moved
  ...
  if ( (!inSleepMode) && (((time2sleep > 0) && (goneMinutes >= time2sleep)) || isOnStand)) {inSleepMode = true; beep();}
...
// isOnStand = HandleDistance >= 25 ? false : false; //check is handle on stand
...
void InfoScreen() {
  ...
  do {
    ...
    uint16_t iHandleDistance = getHandleDistance();
    u8g.firstPage();
      do {
        u8g.setFont(u8g_font_9x15);
        u8g.setFontPosTop();
        ...
        u8g.setPrintPos(0, 48); u8g.print(F("Handle: ")); u8g.print(iHandleDistance);
      } while(u8g.nextPage());
    if (lastbutton && digitalRead(BUTTON_PIN)) {delay(10); lastbutton = false;}
  } while (digitalRead(BUTTON_PIN) || lastbutton);
...
uint16_t getHandleDistance() {
  uint16_t result;
  result = denoiseAnalog (ONSTAND_PIN);
  return result;
}
